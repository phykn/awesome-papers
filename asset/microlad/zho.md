...
- **公式**: $\mathcal{L}_{\text{SDS}} = \kappa(t) \|\epsilon - \epsilon_\theta(z_{\text{slice},t}, t)\|^2$, 其中 $\kappa(t) = \frac{1 - \bar{\alpha}_t}{\bar{\alpha}_t}$
...
- **变量**:
  - $\epsilon_\theta$ = 提供扩散先验信息的冻结的预训练去噪网络 (U-Net) (第 3.4 节 / 式 39)。
  - $z_{\text{slice}}$ = 由 VAE 编码器 $E$ 编码得到的 2D 截面的潜在表示 (第 3.4 节 / 式 37)。
...
- [SliceGAN: 通过基于 GAN 的维度扩展从 2D 切片生成 3D 结构](https://arxiv.org/abs/1903.12564) - 2D 到 3D 微观结构生成的基于 GAN 的基准。
- [Latent Diffusion Models (Stable Diffusion)](https://arxiv.org/abs/2112.10752) - MicroLad 所基于的核心潜在扩散架构。
- [Differentiable Physics Simulation for Materials Design](https://doi.org/10.1016/j.cma.2023.116000) - 通向基于物理信息的材料结构梯度优化的方法。
