...
#### 4.3 核心公式
- **公式**: $C(\mathbf{r}) = \int_{t\_n}^{t\_f} T(t) \sigma(\mathbf{r}(t)) \mathbf{c}(\mathbf{r}(t), \mathbf{d}) dt$, ただし $T(t) = \exp\left(-\int_{t\_n}^t \sigma(\mathbf{r}(s)) ds\right)$
...
- **変数**:
  ...
  - $T(t)$: $t\_n$ から $t$ まで光線に沿って蓄積された透過率 (公式3 / セクション4)。
