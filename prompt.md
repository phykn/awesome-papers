...
- **Markdown robust formatting**:
  ...
  - If an inline math expression contains multiple underscores (`_`), escape them with a backslash (`\_`) to prevent Markdown parsers from interpreting them as italics (e.g., `$\mathcal{L}\_{\text{SDS}}$` instead of `$\mathcal{L}_{\text{SDS}}$`).
